...
class MatchDetail extends Model
{
    ...
    public static function getMatchDetailsById($id = 0, $period=0, $teamId = 0,$playerId = null){
        ...
        DB::raw("AVG(avg_speed_km) as avg_speed_km"),
        DB::raw("MAX(max_speed_km) as max_speed_km"),
        DB::raw("MAX(max_acceleration) as max_acceleration"),
        ...
        ->with('players')->where('is_summary', 0);
        ...
        if($playerId){
            $query = $query->where('player_id',$playerId)->groupBy('match_id');

        }else{
            $query = $query->groupBy('player_id');
        }

        return $query->orderBy('player_id', 'ASC')->get()->toArray();
    }
    ...
    public static function getMatchPlayers($matchId,$teamId = null,$playerId = false){
        ...
        $playerDetails = self::with(['matchStats','distancePerZone','distancePerSprint','timeIntensity'])
                ...
                ->join('matches as mt','mt.id','match_details.match_id')
                ->select('p.id as player_id','p.first_name','p.last_name','mt.name as mt_name' ,'mt.match_date')
                ->where('match_details.match_id',$matchId);

        if($teamId){
            $playerDetails = $playerDetails->where('team_id',$teamId);
        }
